...
temp <- list.files(path = "./data_clean/",
               ...
               full.names = TRUE) %>%
  map_df(~read_excel(.)) %>% 
  mutate(riesgo = factor(riesgo, levels = c("Bajo", 
                                   "Medio", 
                                   "Alto", 
                                   "Vencido"
                                   )
...
evolucion_ret_plot <- temp %>% 
  ...
  dplyr::summarise(n = n()) %>% 
  dplyr::mutate(variacion = n - lag(n),
...
variacion_plot <- rbind(variacion_vigentes, variacion_retrasos) %>% 
  ...
  geom_path(size = 1, alpha = 0.5,  linetype = "dotted") +
  geom_point(size = 1.5, alpha = 0.8) +
  geom_line(aes(corte, media_movil), size = 2) +
...
El índice de resolución corresponde a la relación entre las garantías que se generan y aquellas que se van resolviendo para cada patología.\

Interpretación de los valores:

- Positivos: se resuelven más garantías de los que ingresan.
- Neutros: se resuelve la misma cantidad de las que ingresan.
- Negativos: se resuelven menos garantías de las que ingresan.


```{r echo=FALSE, fig.height=9, fig.width=13, message=FALSE, warning=FALSE, out.width="100%"}
indice_resolucion <- temp %>%
  dplyr::group_by(problema_de_salud, corte) %>% 
  dplyr::summarise(n = n()) %>% 
  dplyr::mutate(variacion = (n - lag(n)) * -1,
                corte = ymd(corte),
                indice = median(variacion, na.rm = TRUE),
                tipo = case_when(
                  indice > 0 ~ "Positivo",
                  indice == 0 ~ "Neutro",
                  indice < 0 ~ "Negativo"
  ...
  ggplot(aes(indice, problema_de_salud, color = tipo,
             text = paste("Indice:", indice, "\nProblema:", problema_de_salud, "\nValor:", tipo))) +
  geom_point(size = 1.2) +
  scale_color_manual("", values = c("#c51b8a", "#cccccc", "#31a354")) +
  theme_bw() +
  labs(x = "Índice de resolución",
       y = "")

ggplotly(indice_resolucion, tooltip = "text")
...
Se muestra la media en los días de retraso y la desviación estándar para cada etapa asistencial.
...
retrasos_etapa <- total_ges |> 
  ...
  dplyr::summarise(dias = mean(dias_retraso),
                   n = n(),
                   sd = sd(dias_retraso)) |> 
  ggplot(aes(fct_reorder(etapa, dias), dias,
             text = paste("Media:", round(dias, 1), "días", "\nDesv. Est.:", round(sd, 1), "\nN° garantías:", n, "\nEtapa:", etapa))) +
  geom_col(fill = "#756bb1") +
  geom_errorbar(aes(x = etapa, ymin = dias - sd, ymax = dias + sd), width = 1, colour = "#f768a1", alpha = 0.9, size = 1.5) +
  coord_flip() +
  theme_bw() +
  labs(
  x = "",
...
plot_3 <- total_ges |> 
  ...
  #scale_color_manual(values = c("#8dd3c7", "#ffed6f", "#bebada", "#fb8072", "#80b1d3", "#fdb462", "#b3de69", "#d9d9d9")) +
  theme_bw() +
...
evolucion_riesgo <- temp %>% 
  ...
  scale_color_manual("Riesgo", values = c("#66c2a5", "#ffd92f", "#fb8072", "#756bb1")) +
  theme_bw() +
  labs(x = "",
       y = "N° de garantías") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
